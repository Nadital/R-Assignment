...
plot(Maize_Chromsome_No,Maize_Total_SNPs, xlab="Maize Chromosome No", ylab = "Total SNPs")
```
...
Next, I created a new files to indicate whether a site was homozygous or heterozygous, as well as any missing data that is coded as NA.
...
Missing_mz <- filter(melt_zygos_mz, SNPs== "NA")
...
Missing_teo <- filter(melt_zygos_teo, SNPs== "NA")

```

I usedd rbind() to combine all the rows into a single file. 
